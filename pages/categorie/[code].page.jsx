...
import { getTypeProperties, getTypePropertiesId } from "@services/properties";
...
export default function ListBien({ data }) {
  ...
  return (
    <Layout>
      <Box
        display={["initial", "initial", "initial", "initial", "flex"]}
        justifyContent="center"
        flexDirection="column"
        alignItems="center"
        mb="20px"
      >
        <Heading as="h1" textAlign="center" margin={[10, 10, 10, 10, 20]}>
          Vos hébergements Atypiques: {data.title}
        </Heading>
        ...
        <SimpleGrid
          ...
          maxWidth="1800px"
        >
          {data?.properties?.map((habitation, i) => (
            <Box
            ...
            >
              ...
              <Box display="flex" justifyContent="space-around" h="120px">
                ...
                >
                  <Text
                    ...
                    lineHeight="short"
                  >
                    {habitation.address.region}
                  </Text>
                  <Text fontSize="xl" fontWeight="semibold" lineHeight="short">
                    {habitation.maxTravelers} personnes
                  </Text>
                  <Text fontSize="xl" fontWeight="semibold" lineHeight="short">
                    {habitation.rate} €/nuit
                  </Text>
                </Box>
                <Box
                  display="flex"
                ...
                >
                  ...
                  {habitation?.accessHandicap === true ? (
                    <Box w="50px">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="100%"
              ...
              <Link href={`/bien/${habitation["@id"].split("/")[3]}`} passHref>
                <Box
                  display="flex"
                  justifyContent="center"
                  alignItems="center"
...
export async function getStaticProps(context) {
  ...
  try {
    const { data } = await getTypePropertiesId(id);
