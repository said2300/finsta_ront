...
const SearchBien = () => {
  ...
  const { isLoading, isError, data: bien, refetch } = useQuery(
    ...
    () =>
      queries?.start &&
      axios.get(
        `https://arcane-spire-42874.herokuapp.com/api/properties/search?dateStart=${queries.start}&dateEnd=${queries.to}&maxTraveler=${queries.voyageur}&town=${queries.ville}`
      ),
      ...
      <Box minHeight="70vh">
        ...
        >
          <Text fontSize="20px" fontWeight="bold" textAlign="center">
            Du {queries.start} au {queries.to} pour {queries.voyageur} personnes
            à {queries.ville}
          </Text>
        </Box>
        <SimpleGrid
          ...
          maxWidth="1800px"
        >
          {!isLoading && bien?.data["hydra:member"].length > 0 ? (
            ...
            <Text fontSize="22px">
              Il n'y a pas de bien disponible pour cette recherche
            </Text>
          )}
        </SimpleGrid>
      </Box>
